...
public enum TesticleSize {
	...
	ZERO_VESTIGIAL("рудиментарные", 0, PresetColour.GENERIC_SIZE_ONE),

	/**Small for a human.*/
	ONE_TINY("маленькие", 1, PresetColour.GENERIC_SIZE_TWO),
	
	/**Average for a human.*/
	TWO_AVERAGE("средние", 2, PresetColour.GENERIC_SIZE_THREE),

	/**Large for a human. Average for a dog or wolf-morph.*/
	THREE_LARGE("большие", 3, PresetColour.GENERIC_SIZE_FOUR),

	/**Unrealistically large for a human. Average for a horse-morph.*/
	FOUR_HUGE("очень большие", 4, PresetColour.GENERIC_SIZE_FIVE),

	/**Large for a horse-morph.*/
	FIVE_MASSIVE("массивные", 5, PresetColour.GENERIC_SIZE_SIX),

	/**Getting pretty absurd.*/
	SIX_GIGANTIC("гигантские", 6, PresetColour.GENERIC_SIZE_SEVEN),

	/**"Extreme proportion" content.*/
	SEVEN_ABSURD("абсурдно огромные", 7, PresetColour.GENERIC_SIZE_EIGHT);
